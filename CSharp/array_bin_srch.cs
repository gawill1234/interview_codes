...
namespace arrSort {
   ...
   class arrOrder {
       ...
       */
      static int ARRSZ = 1024;
      
      static int expected_lookups() {
      int zim = 2;
      int count = 1;
       ...
       *   fail.
       */
      static void init_array(int[] elemarr) {
      int i, j;
       ...
       */
      static int find_between(int curr, int last, int updown, int arrmax) {
      int newvalue = 0;
      int useit;
       ...
       */
      static int find_value(int value, int[] elemarr, int arrsz) {
      int start, Found, last, oldstart, nmax, count;
          ...
          Console.Write("LOOK UPS: " + count + ", ");
          return(start);
      }
      
      static void dump_array(int[] elemarr) {
      int i;
      
         for (i = 0; i < ARRSZ; i++) {
            Console.Write(elemarr[i]);
         }
         Console.WriteLine("\n");
         return;
      }
      ...
      int loc, i;
         ...
         for (i = -5; i < ARRSZ; i += 43) {
            Console.Write("SEEKING: " + i + ",  ");
            loc = find_value(i, elemarr, ARRSZ);
            if (loc >= 0) {
               Console.WriteLine("loc: " + loc + ",  value: " + elemarr[loc]);
            } else {
         ...
         Console.WriteLine("EXPECTED LOOK UPS was: " + expected_lookups() + " for an array of size " + ARRSZ);
